...
**风险：** 4096×4096 纹理文件较大（约 64MB 未压缩，PNG 压缩后约 2-4MB）

**缓解措施：**
- 使用 PNG 压缩，实际文件大小可控
- 4096×4096 纹理在现代设备上兼容性好（OpenGL ES 3.0+、WebGL 2.0+）
- 单纹理设计减少纹理切换开销
...
### Phase 2: TUI 渲染支持
1. 扩展 `symbols.png` 到 4096×4096，包含 Sprite/TUI/Emoji/CJK 四个区域
...
5. **容量充足** - 4096×4096 纹理，CJK 区域可容纳 4,096 个汉字（128列×32行）
...
- ✅ 单纹理单 Pass，性能最优
- ✅ 架构简单一致
- ⚠️ 纹理尺寸 4096×4096（约 64MB 未压缩，PNG 压缩后约 2-4MB）
- ⚠️ 需要预先确定使用的汉字
