...
- 数字：0-9
- 标点符号：.,;:!?
- **特点**：通常高度大于宽度，16x32 渲染效果更好

### 2. 单色图形符号（需要 16x16 正方形）
- 框线字符：`│─┐┌└┘├┤┬┴┼╭╮╰╯`
- 方块字符：`█▀▄▌▐░▒▓`
- 几何形状：`●○◆◇★☆`
- 箭头符号：`←→↑↓↖↗↘↙`
- **特点**：设计为正方形，用 16x32 渲染会被拉长变形
- **渲染**：单色，使用前景色/背景色

### 3. Emoji 表情符号（需要 32x32 彩色图片）
- 食物：🍭 🍕 🍔 🍰
- 表情：🤔 😊 😂 🥺
- 符号：🌟 ⭐ ✨ 💫
- 自然：🍀 🌸 🌈 🔥
...
for cell in buffer {
    if cell.is_emoji() {
        // 使用 Emoji 渲染路径（字体或纹理）
        render_emoji(cell.emoji_char, 32, 32);  // 32x32 正方形
        skip_next_cell = true;
...
### 方案 A：接受拉伸（最简单）
**实现：** 所有 Main Buffer 字符都用 TUI 区域（16x32）渲染
...
```
2048x2048 统一纹理：
- TUI 文本区：0-255px（16x32 瘦高文本）
- TUI 图形区：256-384px（16x16 正方形图形）
- Sprite 区：384-2047px（16x16 游戏精灵）
```
...
### 方案 D：统一正方形（放弃 16x32）
**实现：** TUI 也使用 16x16 字符，放弃 2:1 宽高比
...
**方案 B（智能检测）**
- ✅ 文本字符 → TUI 区域（16x32 瘦高）
- ✅ 图形符号 → Sprite 区域（16x16 正方形，垂直居中）
- ✅ 最佳视觉效果
- 🚫 **Emoji 仍然不支持**
...
|---------|------|------|---------|---------|-------|
| **文本字符** | `Abc123` | 1格 | 1:2 (16x32) | TUI 区域，单色 | P0 ✅ |
| **图形符号** | `│─┌┐●` | 1格 | 1:1 (16x16) | Sprite 区域，单色 | P1 🎯 |
| **Emoji** | `🍭🤔🌟` | 2格 | 1:1 (32x32) | 预制纹理，彩色 | P2 🔮 |
