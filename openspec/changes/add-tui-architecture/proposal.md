...
- **图形模式分层架构**：
  - Main Buffer 专用于 TUI 渲染，使用瘦高字符（1:2，16x32 像素）
  - Pixel Sprites 用于游戏对象，使用正方形字符（1:1，16x16 像素）
  - TUI 层永远渲染在最上层，确保界面始终可见
- **统一坐标系统**：鼠标事件按 16 像素宽度计算，水平方向通用，TUI 层垂直方向除以 2
- **统一符号纹理**：使用 4096x4096 `symbols.png` 包含 Sprite（16x16）、TUI（16x32）、Emoji（32x32 彩色）、CJK（32x32）四个区域
...
- Affected code:
  ...
  - `assets/pix/symbols.png` - 扩展为 4096x4096 统一纹理（Sprite + TUI + Emoji + CJK）
