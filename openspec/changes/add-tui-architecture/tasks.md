...
- [ ] 1.1 创建或扩展符号纹理为 2048x2048 (`assets/pix/symbols.png`)，包含三个区域：
  - 行 0-255（256px）：TUI 符号（8x16 瘦高字符）
  - 行 256-511（256px）：预制 Emoji（16x16 彩色图像）
  - 行 512-2047（1536px）：Sprite 符号（8x8 方形字符）
...
- [ ] 1.6 在 `cell.rs` 中创建 `EMOJI_MAP: HashMap<String, u16>`，映射常用 Emoji 到纹理索引
  - 选择 200-500 个最常用 Emoji（表情、符号、食物、自然、对象等）
  - Emoji 索引范围：4096-6143（Emoji 区域）
...
- [ ] 1.8 在 `graph.rs` 中实现 `render_helper_emoji` 函数
  - 计算公式：`relative_idx = emoji_idx - 4096`, `x = (relative_idx % 128) * 16`, `y = 256 + (relative_idx / 128) * 16`
  - Destination 宽度为 `cell_width * 2.0`（占 2 格）
  - Source 尺寸为 16x16 像素
- [ ] 1.9 在 `render_helper` 中实现 Sprite 区域索引计算（符号索引 6144-55295，对应行 512-2047）
  - 计算公式：`block_x = texidx % 16`, `block_y = texidx / 16`, `pixel_y = 512 + block_y * 128 + char_y * 8`
  - 符号索引 = `6144 + ((pixel_y - 512) / 8) * 256 + (pixel_x / 8)`
...
- [ ] 1.20 更新 UI 组件使用 `column_tui`、`row_tui` 进行鼠标事件处理，确保 TUI 交互正确
...
- [ ] 2.8 验证 Emoji 映射表正确识别常用 Emoji（200-500 个）
...
- [ ] 3.3 创建 Emoji 使用指南文档
  - 说明预制 Emoji 的选择标准（200-500 个常用 Emoji）
  - 列出所有支持的 Emoji 及其分类
  - 说明如何在 TUI 中使用 Emoji
  - 说明未预制 Emoji 的显示行为
- [ ] 3.4 更新鼠标事件处理文档，说明双坐标系统用法
