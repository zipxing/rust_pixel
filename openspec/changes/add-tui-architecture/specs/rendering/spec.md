...
### Requirement: 统一符号纹理与三区域布局

渲染系统 MUST 支持统一的 1024x1024 符号纹理（`symbols.png`），包含三个区域：TUI 符号（8x16 像素）、Emoji（16x16 彩色）、Sprite 符号（8x8 像素）。

#### 场景：统一纹理布局
- **当** 系统加载符号纹理时
- **则** 纹理尺寸为 1024x1024 像素
- **且** 顶部 128 像素（行 0-127）包含 TUI 符号，每个字符 8x16 像素
- **且** 中部 64 像素（行 128-191）包含 Emoji，每个 Emoji 16x16 像素
- **且** 底部 832 像素（行 192-1023）包含 Sprite 符号，每个字符 8x8 像素

#### 场景：TUI 符号区域布局
- **当** 解析 TUI 区域（行 0-127）时
- **则** 该区域每行包含 128 个字符（1024 / 8）
- **且** 每个字符占用 8 像素宽 × 16 像素高
- **且** TUI 字符总数为 128 × 8 = 1024 个字符
- **且** TUI 符号在统一符号数组中占用索引 0-1023

#### 场景：Emoji 区域布局
- **当** 解析 Emoji 区域（行 128-191）时
- **则** 该区域每行包含 64 个 Emoji（1024 / 16）
- **且** 每个 Emoji 占用 16 像素宽 × 16 像素高
- **且** 区域包含 4 行，总计 256 个 Emoji 位置
- **且** Emoji 在统一符号数组中占用索引 1024-1279
- **且** Emoji 使用 RGBA 彩色格式

#### 场景：Sprite 符号区域布局
- **当** 解析 Sprite 区域（行 192-1023，高 832 像素）时
- **则** 该区域每行包含 128 个字符（1024 / 8）
- **且** 每个字符占用 8 像素宽 × 8 像素高
- **且** Sprite 字符总数为 128 × 104 = 13,312 个字符
- **且** Sprite 符号在统一符号数组中占用索引 1280-14591
...
- **且** 预制 175 个最常用 Emoji（表情、符号、食物、自然等）+ 81 个预留空间
- **且** Emoji 索引范围为 1024-1279（Emoji 区域）
...
#### 场景：Emoji 纹理坐标计算
- **当** 渲染 Emoji（索引 1024-1279）时
- **则** 计算公式：`relative_idx = emoji_idx - 1024`
- **且** 纹理 X 坐标 = `(relative_idx % 64) * 16`
- **且** 纹理 Y 坐标 = `128 + (relative_idx / 64) * 16`
- **且** 源纹理尺寸为 16x16 像素
...
- **当** 选择预制 Emoji 集时
- **则** 包含表情与情感类（😀😊😂🤔😭🥺等，约 50 个）
- **且** 包含符号与标志类（✅❌⚠️🔥⭐🌟等，约 30 个）
- **且** 包含箭头与指示类（➡️⬅️⬆️⬇️等，约 20 个）
- **且** 包含食物与饮料类（🍕🍔🍩🍰🍭等，约 20 个）
- **且** 包含自然与动物类（🌈🌸🍀🐱🐶等，约 20 个）
- **且** 包含对象与工具类（📁📂📊🔧💻等，约 20 个）
- **且** 包含活动与运动类（⚽🏀🎮🎯等，约 15 个）
- **且** 预留空间约 81 个，供用户自定义或未来扩展
...
- **则** 使用 TUI 区域的索引计算公式
- **且** 计算公式为：`char_x = symidx % 128, char_y = symidx / 128, pixel_x = char_x * 8, pixel_y = char_y * 16`
- **且** 符号索引 = `symidx`（直接线性索引）
- **且** 最终索引范围为 0-1023

#### 场景：Emoji 层符号索引计算
- **当** 渲染 Emoji 字符时
- **则** 使用 Emoji 区域的索引计算公式
- **且** 计算公式为：`relative_idx = emoji_idx - 1024`
- **且** 纹理坐标：`emoji_x = (relative_idx % 64) * 16, emoji_y = 128 + (relative_idx / 64) * 16`
- **且** 最终索引范围为 1024-1279

#### 场景：Sprite 层符号索引计算
- **当** 渲染 Pixel Sprites 的字符时
- **则** 使用 Sprite 区域的索引计算公式
- **且** 计算公式为：`relative_idx = symidx - 1280, char_x = relative_idx % 128, char_y = relative_idx / 128, pixel_x = char_x * 8, pixel_y = 192 + char_y * 8`
- **且** 符号索引 = `symidx`（直接线性索引）
- **且** 最终索引范围为 1280-14591
