...
渲染系统必须支持统一的 2048x2048 符号纹理（`symbols.png`），包含三个区域：TUI 符号（8x16 像素）、Emoji（16x16 彩色）、Sprite 符号（8x8 像素）。

#### 场景：统一纹理布局
- **当** 系统加载符号纹理时
- **则** 纹理尺寸为 2048x2048 像素
- **且** 顶部 256 像素（行 0-255）包含 TUI 符号，每个字符 8x16 像素
- **且** 中部 256 像素（行 256-511）包含 Emoji，每个 Emoji 16x16 像素
- **且** 底部 1536 像素（行 512-2047）包含 Sprite 符号，每个字符 8x8 像素

#### 场景：TUI 符号区域布局
- **当** 解析 TUI 区域（行 0-255）时
...
#### 场景：Emoji 区域布局
- **当** 解析 Emoji 区域（行 256-511）时
- **则** 该区域每行包含 128 个 Emoji（2048 / 16）
- **且** 每个 Emoji 占用 16 像素宽 × 16 像素高
- **且** 区域包含 16 行，总计 2048 个 Emoji 位置
- **且** Emoji 在统一符号数组中占用索引 4096-6143
- **且** Emoji 使用 RGBA 彩色格式

#### 场景：Sprite 符号区域布局
- **当** 解析 Sprite 区域（行 512-2047，高 1536 像素）时
- **则** 该区域包含 16 列 × 12 行区块
- **且** 每个区块包含 16x16 个字符，每个字符 8x8 像素
- **且** Sprite 字符总数为 16 × 12 × 16 × 16 = 49,152 个字符
- **且** Sprite 符号在统一符号数组中占用索引 6144-55295
...
渲染系统必须支持预制 Emoji 的映射、渲染和双宽字符（wcwidth=2）处理，为 TUI 模式提供常用 Emoji 显示能力。

#### 场景：Emoji 映射表
- **当** 系统初始化时
- **则** 创建 `EMOJI_MAP: HashMap<String, u16>`，将 Emoji 字符映射到纹理索引
- **且** 预制 200-500 个最常用 Emoji（表情、符号、食物、自然等）
- **且** Emoji 索引范围为 4096-6143（Emoji 区域）
...
#### 场景：Emoji 纹理坐标计算
- **当** 渲染 Emoji（索引 4096-6143）时
- **则** 计算公式：`relative_idx = emoji_idx - 4096`
- **且** 纹理 X 坐标 = `(relative_idx % 128) * 16`
- **且** 纹理 Y 坐标 = `256 + (relative_idx / 128) * 16`
- **且** 源纹理尺寸为 16x16 像素
...
渲染系统必须根据渲染层（TUI、Emoji 或 Sprite）自动计算正确的纹理符号索引，确保从统一纹理的正确区域采样。
...
- **且** 计算公式为：`relative_idx = emoji_idx - 4096`
- **且** 纹理坐标：`x = (relative_idx % 128) * 16, y = 256 + (relative_idx / 128) * 16`
- **且** 最终索引范围为 4096-6143

#### 场景：Sprite 层符号索引计算
- **当** 渲染 Pixel Sprites 的字符时
- **则** 使用 Sprite 区域的索引计算公式
- **且** 计算公式为：`pixel_y = 512 + block_y * 128 + char_y * 8, pixel_x = block_x * 128 + char_x * 8`
- **且** 符号索引 = `6144 + ((pixel_y - 512) / 8) * 256 + (pixel_x / 8)`
- **且** 最终索引范围为 6144-55295
