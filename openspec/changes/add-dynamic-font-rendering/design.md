...
- **TUI 区域** (Block 48-52): 640 个 16x32 像素字符，用于 ASCII + 常用符号 → **将被移除**
- **Emoji 区域** (Block 53-55): 192 个 32x32 像素 Emoji

**问题：**
1. TUI 区域的 640 个预渲染字符无法覆盖 CJK 字符集（2万+字符）
2. 预渲染位图在缩放时会模糊或出现锯齿

**解决方案：** 移除 TUI 预渲染区域，所有 TUI 文本字符改用动态字体光栅化。
...
- 混合模式:静态图集(Sprite/Emoji) + 动态字体(所有文本字符)
- 移除 TUI 预渲染区域,简化架构
- 预留彩色 Emoji 升级接口
...
4. **~~TUI 预渲染区域~~** (Block 48-52): **移除**，不再使用

**为什么移除 TUI 预渲染区域 (Block 48-52)：**
- **简化架构**: 减少一个字符来源，代码更简洁
- **释放空间**: Block 48-52 可用于扩展 Sprite 或 Emoji
- **无冗余**: 既然所有文本都用动态渲染，预渲染区域就没有存在意义
- **一致性**: 所有文本字符走同一渲染路径
...
fn get_glyph_source(ch: char, symbol: &str) -> GlyphSource {
    ...
    // 注：不再检查 tui_char_index()，Block 48-52 已移除
...
**渲染流程：**
```
第一次 Draw Call: 静态图集 (symbols.png)
├─ Sprite 符号 (Block 0-47)
└─ Emoji (Block 53-55)

第二次 Draw Call: 动态图集 (dynamic_glyphs)
└─ 所有 TUI 文本字符 (ASCII + CJK + Unicode)
```

**为什么不追求一次渲染：**
- **架构简单**: 两个独立纹理，各自管理
- **代码改动小**: 现有渲染管线几乎不用动
- **性能影响极小**: 现代 GPU 两次 draw call 开销可忽略（< 0.1ms/frame）
- **调试方便**: 可以单独禁用动态渲染来排查问题

**实现方式：**
```rust
fn render(&mut self) {
    // 1. 收集所有需要渲染的 cells，按纹理分类
    let (static_cells, dynamic_cells) = self.categorize_cells();

    // 2. 第一次渲染：静态图集 (Sprite + Emoji)
    self.bind_texture(self.symbols_texture);
    self.draw_batch(&static_cells);

    // 3. 第二次渲染：动态图集 (所有文本字符)
    if !dynamic_cells.is_empty() {
        self.bind_texture(self.glyph_renderer.dynamic_atlas.texture_id);
        self.draw_batch(&dynamic_cells);
    }
}
```

**性能对比：**
| 方案 | Draw Calls | 复杂度 | 维护成本 |
|------|-----------|--------|----------|
| 一次渲染 | 1 | 高（需 multi-texture） | 高 |
| 两次渲染 | 2 | 低 | 低 |

### 决策 4: RenderCell 结构完全不改

**现有 RenderCell 结构：**
```rust
pub struct RenderCell {
    pub fcolor: (f32, f32, f32, f32),      // 前景色 RGBA
    pub bcolor: Option<(f32, f32, f32, f32)>, // 背景色
    pub texsym: usize,                      // 纹理索引 + 符号索引
    pub x: f32, pub y: f32,                 // 屏幕位置
    pub w: u32, pub h: u32,                 // 尺寸
    pub angle: f32, pub cx: f32, pub cy: f32, // 旋转
    pub modifier: u16,                       // 样式修饰符
}
```

**方案选择：两个独立的 `Vec<RenderCell>`，结构完全不改**

```rust
// generate_render_buffer() 输出两个数组
fn generate_render_buffer() -> (Vec<RenderCell>, Vec<RenderCell>) {
    let mut static_cells = Vec::new();   // Sprite + Emoji
    let mut dynamic_cells = Vec::new();  // TUI 文本字符
    // ...
    (static_cells, dynamic_cells)
}

// draw_render_buffer_to_texture() 两次渲染到同一个 Texture 2
fn draw_render_buffer_to_texture(
    static_cells: &[RenderCell],
    dynamic_cells: &[RenderCell],
    target: usize  // 仍然是 2
) {
    // Pass 1: 静态图集
    self.bind_texture(symbols_texture);
    self.draw_batch(static_cells);

    // Pass 2: 动态图集
    if !dynamic_cells.is_empty() {
        self.bind_texture(dynamic_glyphs_texture);
        self.draw_batch(dynamic_cells);
    }
}
```
...
**texsym 字段的语义：**
- `static_cells.texsym`: 指向 `symbols.png` 的 Block 索引（现有编码）
- `dynamic_cells.texsym`: 指向 `dynamic_glyphs` 的槽位索引（新编码）
- 两个数组分开渲染，各自的 `texsym` 语义独立，互不干扰

**为什么不改 RenderCell：**
- **最小改动原则**: 现有结构已经足够表达所有渲染信息
- **二进制兼容**: 不影响 FFI/WASM 导出的数据格式
- **简单可靠**: 分开两个数组比扩展字段更清晰

**备选方案（未采用）：**
```rust
// 方案 B: 用 texsym 最高位标记纹理类型
const DYNAMIC_TEXTURE_FLAG: usize = 1 << 31;
texsym = DYNAMIC_TEXTURE_FLAG | glyph_slot_idx;
```
- 缺点: 需要位运算判断，增加渲染时的分支
...
**变更前 (symbols.png 2048x2048):**
```
Block 0-47:  Sprite 符号 (3,072 个 16x16)
Block 48-52: TUI 预渲染字符 (640 个 16x32)  ← 移除
Block 53-55: Emoji (192 个 32x32)
```

**变更后:**
```
symbols.png (2048x2048):
├─ Block 0-47:  Sprite 符号 (保持不变)
├─ Block 48-52: [空闲/可扩展]  ← 释放
└─ Block 53-55: Emoji (保持不变)

dynamic_glyphs (1024x1024 @ 1x, 随 DPI 缩放):
└─ 所有 TUI 文本字符 (ASCII + CJK + Unicode)
    1024 个 32x32 槽位, LRU 管理
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. categorize_cells() 按纹理分类                            │
│     ├─ static_cells:  Sprite + Emoji                        │
│     └─ dynamic_cells: 所有文本字符                           │
│                                                             │
│  2. 第一次 Draw Call: 静态图集                               │
│     bind_texture(symbols.png)                               │
│     draw_batch(static_cells)                                │
│                                                             │
│  3. 第二次 Draw Call: 动态图集                               │
│     bind_texture(dynamic_glyphs)                            │
│     draw_batch(dynamic_cells)                               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
```
render_helper_tui(cell):
┌─────────────────────────────────────────────────────────────┐
│  1. is_sprite_symbol(ch)?  ──→ SpriteAtlas (Block 0-47)     │
│  2. emoji_texidx(symbol)?  ──→ EmojiAtlas (Block 53-55)     │
│  3. else                   ──→ Dynamic (动态图集)           │
│     - ASCII (0x20~0x7E)                                      │
│     - CJK (中日韩汉字)                                       │
│     - 其他 Unicode 字符                                      │
...
pub enum GlyphSource {
    ...
    /// 动态光栅化字形 (dynamic_glyphs 纹理)
    Dynamic { texture_id: u32, uv: UVRect },
}
// 注：TuiAtlas 已移除，Block 48-52 不再使用
...
### Trade-off: 两次渲染 vs 一次渲染
- **选择:** 两次 Draw Call（静态图集 + 动态图集）
- **原因:**
  - 架构简单，两个独立纹理各自管理
  - 代码改动小，现有渲染管线几乎不用动
  - 调试方便，可单独禁用动态渲染
- **影响:** 额外一次 draw call，开销约 0.05-0.1ms/frame，可忽略

### Trade-off: 移除 TUI 预渲染区域
- **选择:** 完全移除 Block 48-52
- **原因:**
  - 所有文本字符都用动态渲染，预渲染区域无用
  - 简化代码，减少一个字符来源的判断
  - 释放空间可用于扩展 Sprite 或 Emoji
- **影响:** 需要更新 symbols.png 相关代码，但整体更简洁
