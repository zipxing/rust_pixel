...
fn main() {
    ...
    let blocks = match choice.trim() {
        "1" => {
            print!("{}请输入方块数量: {}", BOLD, RESET);
            io::stdout().flush().unwrap();
            let mut num_input = String::new();
            io::stdin().read_line(&mut num_input).unwrap();
            let num_blocks = num_input.trim().parse::<u8>().unwrap_or(3);
            create_random_layout(num_blocks)
        }
        "2" => create_manual_layout(),
        "3" => {
            // 创建一个简单的测试布局，便于快速测试
            vec![
                Block {
        ...
        _ => {
            println!("{}无效选择，使用默认随机布局{}", BOLD, RESET);
            create_random_layout(3)
        }
    };
    ...
    println!("\n{}{}开始求解...{}", BOLD, GREEN, RESET);

    // 保存初始布局以便稍后传递给visualize_solution
    let initial_blocks = blocks.clone();

    match solve(blocks, &gates) {
        Some(solution) => {
            println!("sssss.....{:?}", solution);
            println!("\n{}{}找到解决方案！{}", BOLD, GREEN, RESET);
            visualize_solution_with_initial_colored(&solution, &gates, &initial_blocks);
        }
